...
def _run(record: LaunchRecord) -> None:
    ...
    #    In cashback mode the budget stays as SOL; holder rewards come from fees.
    ...
    for cycle in range(_settings.DISTRIBUTION_CYCLES):
...
def _buyback_and_burn(record: LaunchRecord, secret: str, sol_amount: float) -> None:
    """Use `sol_amount` SOL to buy back your $ASSETLY main token and burn it.
    ...
    try:
        ...
        if bought > 0:
            ...
            _log(record, f"Buyback+burn {sol_amount:.4f} SOL -> burned {bought} $ASSETLY")
    except Exception as e:  # noqa: BLE001
